...
test("API dataset filter updates the URL-scoped dataset", () => {
  ...
  assert.match(apiQueueSource, /const QA_QUEUE_DATASET_OPTIONS = \[/);
  assert.match(apiQueueSource, /\{ value: "nuscenes", label: "nuScenes" \}/);
  assert.match(apiQueueSource, /\{ value: "kitti", label: "KITTI" \}/);
  assert.match(apiQueueSource, /DEFAULT_QA_QUEUE_SPLIT = "product"/);
